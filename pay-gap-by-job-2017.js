function PayGapByJob2017() {
  ...
  this.draw = function() {
    ...
    for (i = 0; i < this.data.getRowCount(); i++) {
      ...
      // size = numJobs
      ellipse(
        map(propFemale[i], propFemaleMin, propFemaleMax, this.pad, width - this.pad), // x
        map(payGap[i], payGapMin, payGapMax, height - this.pad, this.pad),           // y
        map(numJobs[i], numJobsMin, numJobsMax, this.dotSizeMin, this.dotSizeMax)   // size
      );
    }
  };
